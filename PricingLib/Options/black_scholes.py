...
def _black_scholes_call_price_single(obj: Option) -> float:
    """Calculate Black-Scholes call price for a single option."""
    time_years = obj.time_to_maturity / DAYS_IN_YEAR
    vol_sqrt_time = obj.volatility * np.sqrt(time_years)

    d1 = (np.log(obj.spot / obj.strike) +
          (obj.rate + 0.5 * obj.volatility ** 2) * time_years) / vol_sqrt_time
    d2 = d1 - vol_sqrt_time

    discount_factor = np.exp(-obj.rate * time_years)
    return obj.spot * stats.norm.cdf(d1) - obj.strike * discount_factor * stats.norm.cdf(d2)


def _black_scholes_put_price_single(obj: Option) -> float:
    """
    Calculate the Black-Scholes price for a European put option.
    Parameters:
        obj (Option): An instance of the Option class containing option parameters.
    Returns:
        float: The calculated put option price.
    """
    call_price = black_scholes_call_price(obj)
    put_price = call_price + obj.strike * np.exp(-obj.rate * (obj.time_to_maturity / DAYS_IN_YEAR)) - obj.spot

    return put_price


def black_scholes_call_price(obj: Union[Option, List[Option]]) -> Union[float, List[float]]:
    """
    Calculate Black-Scholes call price for single option or list of options.
    """
    if isinstance(obj, Option):
        return _black_scholes_call_price_single(obj)
    else:
        # Explicitly type the result
        call_price: List[float] = [_black_scholes_call_price_single(option) for option in obj]
        return call_price


def black_scholes_put_price(obj: Union[Option, List[Option]]) -> Union[float, List[float]]:
    """
    Calculate Black-Scholes put price for single option or list of options.
    """
    if isinstance(obj, Option):
        return _black_scholes_put_price_single(obj)
    else:
        # Explicitly type the result
        put_price: List[float] = [_black_scholes_put_price_single(option) for option in obj]
        return put_price
